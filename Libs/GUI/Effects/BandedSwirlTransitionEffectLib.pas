...
begin
  ...
  if not (IsHandleOf(P, TBandedSwirlTransitionEffect)) then
  begin
    SetError(ERR_INVALID_EFFECT, FuncName + ': not a TBandedSwirlTransitionEffect');
    Exit;
  end;
  Result := True;
end;
...
begin
  ...
  try
    Value := Args[1].n;
    // User passes 0-1, convert to 0-100 for FireMonkey
    if Value <= 1.0 then
      Value := Value * 100;
