...
This is a ready-to-compile FMX host form around the interpreter, which lives
beside it in `../engine/` together with the standard libraries. It makes it easy
to:
...
Open `Plan9BasicApplet.dproj` in RAD Studio and build, or from the command line
in this directory:
...
its path from the `.dpr`, including the ones under `../engine/`.

The interpreter used to arrive here as a git submodule, and the runner kept
building whichever commit the pointer named until someone moved it — so an
engine fix landed here only on a third commit, and forgetting that one produced
no error at all, just an older interpreter. Since 2026-08-19 the engine is a
sibling directory in the same repository, and a fix to it is a fix here.
...
| Tool | Notes |
|------|-------|
| **RAD Studio** or **Delphi** | Version 10.3 Rio or later recommended. FireMonkey is required: this runner has a window, and three engine units still reach FMX (see `tools/check-fmx-boundary.py`). |
...
| **Git** | Nothing special — one repository, one ordinary clone. |
...
Everything below lives in `../engine/`, shared with the Plan9Basic IDE and the
test runner so there is only ever one copy of it. It was its own repository,
[Plan9BasicEngine](https://github.com/AndreMurtaX/Plan9BasicEngine), until
2026-08-19; that repository is still there, but nothing builds from it now.
...
This directory holds the host application. The interpreter and the standard
library sit beside it, under `../engine/`, and are shared with the IDE in the
same repository — so the listing below starts one level up.
...
│
├── engine/                     ← shared with the IDE, and with the test runner
...
│
├── Libs/                       ← the IDE's own libraries, GUI included
├── tests/                      ← the headless suites
└── Plan9Basic.dpr              ← the IDE
```
