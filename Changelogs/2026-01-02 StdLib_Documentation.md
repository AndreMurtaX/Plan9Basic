...
**Parameters:**
- `seconds`: Duration to pause (can include fractions, e.g., 0.5 for half a second)
...
**Important:** Note that `isnil()` returns `1` when the pointer IS valid (assigned), which might seem counterintuitive given the function name. Think of it as "is not nil" or use it as shown in the examples.
...
for i = 1 to total
    ...
    if i mod 10 = 0 then
        println "Progress: "; i; "%"
        processmessages()  ' Keep UI responsive
    endif
next
...
### Program Control
```basic
pause(seconds)        ' Pause execution (can use fractions)
...
isnil(p#)             ' Is pointer assigned? (1=assigned, 0=nil)
