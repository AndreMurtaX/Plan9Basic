...
let client# = http_client#("https://api.example.com")
let client# = http_bearerauth#(client#, apiToken$)

' Build form with text and files
let form# = http_form#()
let form# = http_formfield#(form#, "title", "My Document")
let form# = http_formfield#(form#, "description", "An important file")
let form# = http_formfield#(form#, "category", "reports")
let form# = http_formfile#(form#, "file", "C:\docs\report.pdf")
let form# = http_formfile#(form#, "thumbnail", "C:\images\thumb.png")

' Submit
let response$ = http_postform$(client#, "/api/documents", form#)

if http_ok(client#) <> 0 then
    println "Upload successful!"
    println response$
else
    println "Error: " + stri$(http_status(client#))
end if
```
...
' Use with b64decodefile() from Base64Lib for binary files
let base64$ = http_bodybase64$(client#)
let success = b64decodefile(base64$, "image.png")
...
HttpLib supports event-driven programming with callback functions that are triggered during HTTP operations. This enables progress tracking, error handling, and custom request lifecycle management.

### Setting Callbacks

```basic
' Define a progress callback function
function OnProgress(client#, bytesReceived, totalBytes, abort) local percent
    let percent = int((bytesReceived / totalBytes) * 100)
    println "Download: " + str$(percent) + "%"
end function

' Define a completion callback
function OnComplete(client#, statusCode) local x
    println "Request finished with status: " + str$(statusCode)
end function

' Define an error callback
function OnError(client#, errorMessage$) local x
    println "Error occurred: " + errorMessage$
end function

' Set callbacks on client
let client# = http_client#("https://api.example.com")
let client# = http_onprogress#(client#, "OnProgress")
let client# = http_oncomplete#(client#, "OnComplete")
let client# = http_onerror#(client#, "OnError")
```

### Callback Signatures

The callback functions must have the exact parameter types shown below. The library internally builds the function signature in lowercase (e.g., `myprogress@#nnn`) to match Plan9Basic's UserFunctionsTable.

| Callback | Parameters | Internal Signature |
|----------|------------|-------------------|
| OnProgress | `client#, bytesReceived, totalBytes, abort` | `funcname@#nnn` |
| OnComplete | `client#, statusCode` | `funcname@#n` |
| OnError | `client#, errorMessage$` | `funcname@#$` |

**Important:** Parameter types must match exactly:
- `#` = pointer (client handle)
- `n` = number
- `$` = string

### Getting Callback Names

```basic
' Check what callbacks are set
let progressFunc$ = http_onprogress$(client#)
let completeFunc$ = http_oncomplete$(client#)
let errorFunc$ = http_onerror$(client#)
```
...
let client# = http_clearcallbacks#(client#)

' Or clear individually by setting empty string
let client# = http_onprogress#(client#, "")
```
...
function ShowProgress(client#, received, total, abort) local pct
    let bytesTotal = received
    if total > 0 then
        let pct = int((received / total) * 100)
        println "Downloading: " + str$(pct) + "% complete"
    end if
end function

function DownloadDone(client#, status) local x
    println "Download complete! Total: " + str$(bytesTotal) + " bytes"
end function

let client# = http_client#("https://example.com")
let client# = http_onprogress#(client#, "ShowProgress")
let client# = http_oncomplete#(client#, "DownloadDone")

let data$ = http_get$(client#, "/largefile.zip")
```
...
let form# = http_formfile#(form#, "summary", "C:\Reports\summary.docx")
...
### Example 4: Download with Progress Check

```basic
let client# = http_client#

' First check file size with HEAD
let status = http_head(client#, "https://example.com/largefile.zip")
let fileSize = http_contentlength(client#)
println "File size: " + stri$(fileSize) + " bytes"

' Download
println "Downloading..."
let success = http_download(client#, "https://example.com/largefile.zip", "C:\download.zip")
...
### Event Callbacks (9 functions)
- `http_onprogress#`, `http_onprogress$`
- `http_oncomplete#`, `http_oncomplete$`
- `http_onerror#`, `http_onerror$`
- `http_onredirect#`, `http_onredirect$`
- `http_clearcallbacks#`
...
// In initialization
RegisterHttpFuncs(LibFunctionsTable);
```
